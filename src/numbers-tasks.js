...
function getAverage(value1, value2) {
  const solution = (value1 + value2) / 2;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  return Math.abs(Math.atan2(x2, y2) - Math.atan2(x1, y1));
}
...
function getSumOfDigits(num) {
  ...
  for (let i = 0; ; i += 1) {
    ...
    if (number < 10) return solution + number;
  }
}
...
function toExponential(number, fractionDigits) {
  return Number.parseFloat(String(number)).toExponential(fractionDigits);
}
...
function toFixed(number, fractionDigits) {
  return Number.parseFloat(String(number)).toFixed(fractionDigits);
}
...
function getSumOfNumbers(x1, x2, x3) {
  return Number(toPrecision(x1 + x2 + x3, 2));
}
